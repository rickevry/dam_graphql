...
namespace Dam.Repository
{
    public abstract class MutableEntity
    {
        ...
        public void SetMutableFields<TEntity>(TEntity mutated) where TEntity : MutableEntity
        {
            ...
            foreach (var property in mutatedProperties)
            {
                ...
                switch (mutatedValue)
                {
                    case MutableEntity mutableChild:
        ...
        private void SetMutableChildFields(PropertyInfo property, MutableEntity mutableChild)
        {
            var originalValue = property.GetValue(this) as MutableEntity;
        ...
        private object ExtractEntityId()
        {
            ...
            if (properties != null && properties.Any())
            {
                ...
                var entityName = GetType().GetCustomAttributes(typeof(EntityNameAttribute), false).Select(qn => qn as EntityNameAttribute).FirstOrDefault();
